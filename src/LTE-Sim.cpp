...
#include "scenarios/building.h"
#include "scenarios/single-cell-with-femto-with-riuso.h"
  ...
  if (argc > 1)
    {
	  ...
	  if (strcmp(argv[1], "SingleCellWithI")==0)
       ...
       if (strcmp(argv[1], "SoBuil")==0) /* Made by Rui R. Paulo, please cite  doi: 10.1109/VTCSpring.2018.8417715 , or https://www.it.pt/Publications/DownloadPaperConference/34744 */
	 	 	    {
	 		  double radius = atof(argv[2]);
	 		  ...
	 		  int Power = atoi(argv[18]);
	 		 double LWall =atof(argv[19]);
	 		  int seed;
	 		  if (argc==21) seed = atoi(argv[20]);
	 		  else seed = -1;
	 		  SoBuilding (radius, nbBuilding, buildingType, activityRatio, nbUE, nbFemtoUE, nbVoIP, nbVideo, nbBE, nbCBR, sched_type, frame_struct, speed, accessPolicy, maxDelay, video_bit_rate, Power, LWall, seed);
        }
        if (strcmp(argv[1], "SingleCellWithFemtoWithRiuso")==0) /* Made by Rui R. Paulo, please cite  doi: 10.1109/VTCFall.2018.8690935 */
		{
		  double radius = atof(argv[2]);
		  	          int nbBuilding = atoi(argv[3]);
		  		  int buildingType = atoi(argv[4]);
		  		  ...
		  		  double maxDelay = atof(argv[16]);
		  		  int video_bit_rate = atoi(argv[17]);
		  		  double Power = atof(argv[18]);
		  		  double LWall =atof(argv[19]);
		  		  int seed;
		  		  if (argc==21) seed = atoi(argv[20]);
		  		  else seed = -1;
		  		SingleCellWithFemtoWithRiuso(radius, nbBuilding, buildingType, activityRatio, nbUE, nbFemtoUE, nbVoIP, nbVideo, nbBE, nbCBR, sched_type, frame_struct, speed, accessPolicy, maxDelay, video_bit_rate, Power, LWall, seed);
		}
	 ...
	 if (strcmp(argv[1], "SingleCellWithFemto")==0)
		{
		  ...
		  SingleCellWithFemto(radius, nbBuilding, buildingType, activityRatio, nbUE, nbFemtoUE, nbVoIP, nbVideo, nbBE, nbCBR, sched_type, frame_struct, speed, accessPolicy, maxDelay, video_bit_rate, Power, LWall, seed);
		}
