...
///maybe needed to operate vector concatenation
#include <vector>
#include <iterator>

///Rui Paulo, End 

static void SoBuilding (double radius, int nbBuildings, int buildingType, double activityRatio,
        				int nbUE, int nbFemtoUE,
        				int nbVoIP, int nbVideo, int nbBE, int nbCBR,
        				int sched_type,
        				int frame_struct,
        				int speed, int accessPolicy,
        				double maxDelay, int videoBitRate, double Power, double LWall,
        				int seed)
{
  int nbCell = 1;
  ...
  // define eNB bandwidth and reuse
  int cluster_enb=1;
  int bandwidth_enb=20;
  
  // define Femtocells bandwidth and reuse
  int cluster_henb=2;
  int bandwidth_henb=10;
...
*/
  ...
  // spectrum to eNB
  //std::vector <BandwidthManager*> spectrums = RunFrequencyReuseTechniques (nbCell, cluster_enb, bandwidth_enb);
  
  // spectrum to Femtocells
  std::vector <BandwidthManager*> spectrums = RunFrequencyReuseTechniques (femtoCellsInBuilding, cluster_henb, bandwidth_henb);
  
  //Vector concatenation, zx inside spectrums 
   //spectrums.insert(spectrums.end(), zx.begin(),  zx.end() );
...
/*  std::vector <ENodeB*> *eNBs = new std::vector <ENodeB*>;
  ...
  for (int i = 0; i <  nbFemtoCells; i++)
  {
	  ...
	  if ( HeNBdrop <= activityRatio )
	  {
		  ...
		  HeNodeB* enb = new HeNodeB (i, femtocells->at (i));
		  ...
		  if (accessPolicy == 1)
		  {
			  ...
			  HenbMacEntity* henb_mac = (HenbMacEntity*) enb->GetProtocolStack()->GetMacEntity();
...
/*  int totalNbUE = nbCell*nbUE + nbFemtoCells*nbFemtoUE;
  ...
  int totalNbUE = nbFemtoCells*nbFemtoUE;
  int totalNbCell =  nbFemtoCells;
  ...
  //The old was for (int j = 0; j < nbFemtoCells; j++)
  //Corrected from google groups

  for (int j = 0; j < HeNBs->size(); j++)
      ...
      for (int i = 0; i < nbFemtoUE; i++)
        {
		  ...
		  if (accessPolicy == 1)
		  {
			  ...
			  HenbMacEntity* henb_mac = (HenbMacEntity*) HeNBs->at(j)->GetProtocolStack()->GetMacEntity();
