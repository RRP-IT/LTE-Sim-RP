                       ...
                       int seed)
{
  ...
  int Nbt=nbCell + nbFemtoCells;

  std::vector <BandwidthManager*> spectrums = RunFrequencyReuseTechniques (Nbt, cluster, bandwidth);
 ...
 // std::vector <BandwidthManager*> femto_spectrums = RunFrequencyReuseTechniques (nbCell, cluster, bandwidth);
  ...
  for (int i = nbCell; i < nbCell + nbFemtoCells; i++)
  {
	  ...
	  BandwidthManager* femto_spectrums = spectrums.at(i);
      ...
      for (int i = 0; i < nbFemtoUE; i++)
        {
		  ...
		  // define the channel realization
		  WinnerDownlinkChannelRealization* c_dl = new WinnerDownlinkChannelRealization (HeNBs->at (j), ue);
		  HeNBs->at (j)->GetPhy ()->GetDlChannel ()->GetPropagationLossModel ()->AddChannelRealization (c_dl);
		  WinnerDownlinkChannelRealization* c_ul = new WinnerDownlinkChannelRealization (ue, HeNBs->at (j));
