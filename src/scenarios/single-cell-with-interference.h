...
//double posX = (double)rand()/RAND_MAX; posX = 0.95 * (((2*radius*1000)*posX) - (radius*1000));
//	  double posY = (double)rand()/RAND_MAX; posY = 0.95 * (((2*radius*1000)*posY) - (radius*1000));
/*
	  ...
	  //double posX = 2*radius*1000*0.95*xa-(radius*1000);
	  //double posY = 20*ya-10;

	  int posX = xa*0.95*radius * 1000; //*xa-(radius*1000);
	  int posY = ya*0.95*radius * 1000;
*/
	  ...
	  unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
	  	  	 ...
	  	  	 double radii=dist_double(rng_mt);
	  	  	 double th=theta_double(rng_mt);

	  	  	 double angle = 2*M_PI*th/360;
	  	  	 double r1 = sqrt(radii);
	  	  	 double r = radius*r1;

	  	   int posX = r * cos (angle) * 1000;
	  	 int posY = r * sin (angle) * 1000;
